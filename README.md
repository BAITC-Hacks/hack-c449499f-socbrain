...
**Текущая версия — расширенный прототип:** в репозитории есть Python-модуль распознавания,
HTTP-приёмник записей, отдельные коннекторы Cisco CMS и Zoom, а также статический веб-интерфейс.
Интерфейс пока не соединён с API. Автоматическое составление протокола и извлечение поручений ещё
не реализованы. Данные, статусы и показатели на веб-страницах демонстрационные.
...
| Приём записей | Авторизованная загрузка Cisco/Zoom, идемпотентная SQLite-очередь и локальный STT-worker | Реализовано в [backend/INGEST.md](backend/INGEST.md) |
| Cisco CMS | Календарь CMS Scheduler, SIP-подключение ассистента, запись WAV и доставка в SOCBrain | Реализовано в [integrations/cisco-connector/](integrations/cisco-connector/README.md) |
| Zoom | Cloud Recording, календарь организатора и приглашений, RTMS-аудио и live-транскрипт с именами | Реализовано в [integrations/zoom-connector/](integrations/zoom-connector/README.md) |
...
| API и очереди | FastAPI, SQLite | Приём файлов, webhooks, календарь и восстановление фоновой обработки |
| Коннекторы | Python, Docker Compose, SIP/FFmpeg, официальный Zoom RTMS SDK | Получение записей и live-аудио из Cisco CMS и Zoom |
...
В репозитории нет подключённой LLM для генерации протокола. SQLite используется как техническая
очередь приёма и коннекторов, но бизнес-модель совещаний и поручений пока не реализована. Упоминания
`pyannote.audio`, локальной LLM, SMTP и части корпоративных интеграций в интерфейсе описывают макет.
...
CLI принимает путь к локальному файлу. Подключение к конференциям и получение live-потока выполняют
отдельные коннекторы, а не эта команда.
...
- **Cisco CMS:** расписание Scheduler, SIP-ассистент, запись и доставка в локальный приёмник.
- **Zoom:** Cloud Recording, календарь встреч и приглашений, официальный RTMS SDK для live-аудио,
  транскрипта и имён участников.
- **Teams, Google Meet, СЭД, SMTP, SSO и LDAP:** действующих интеграций в репозитории нет. Часть из них упоминается в техническом задании или макетах настроек.
...
- **Нет сквозного веб-сценария:** API загрузки, обработки и получения результата есть, но статический
  интерфейс к нему ещё не подключён.
- **Нет общей диаризации и голосовой идентификации:** Zoom RTMS передаёт имена и идентификаторы
  участников; для загруженных Cisco/Zoom-записей локальный STT возвращает только время и текст.
...
- **Нет хранения бизнес-данных:** SQLite хранит технические очереди и результаты обработки, но нет
  полноценной базы совещаний и поручений, напоминаний и рассылок. Автоматическое удаление аудио не реализовано.
...
- **Нет обработки длинных записей по частям:** файл передаётся STT-провайдеру одним вызовом. Приёмник
  ограничивает размер и восстанавливает очередь после сбоя, но не делит аудио на фрагменты.
- **Инфраструктура проверена без реальных аккаунтов:** коннекторы имеют тесты, CI, Dockerfile и Docker
  Compose, но полная приёмка требует тестовых Cisco CMS и Zoom с нужными лицензиями и учётными данными.
...
## Коннекторы совещаний

[Cisco CMS Connector и SIP-ассистент](integrations/cisco-connector/README.md) получают расписание
CMS Scheduler, подключаются к разрешённым комнатам, записывают аудио и передают WAV в SOCBrain.
[Zoom Connector](integrations/zoom-connector/README.md) получает Cloud Recording, объединяет календарь
организованных встреч и приглашений, а через официальный RTMS SDK принимает live-аудио и реплики
с именами участников.
[HTTP-приёмник и локальный STT-worker](backend/INGEST.md) принимают файл и возвращают транскрипт
через API. Для реального запуска нужны параметры Cisco/Zoom и заранее загруженная локальная модель.
